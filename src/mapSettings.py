...
class MapSettings(QtGui.QMainWindow):
        ...
        for i in range(len(enableRequest)):
            if( enableRequest[i].startswith('!') ):
                ...
                try:
                    ...
                    self.config["debug"].debugLog("Error: missing keys"+enableRequest[i])
    ...
    def saveMapSetting(self):
        ...
        # -- Extent
        if(self.QMapSettingWindow.mf_map_extent_manuel.isChecked()):
            self.config['debug'].debugLog('Extent: Save Manuel Extent')
            self.config['debug'].debugLog(str(self.QMapSettingWindow.mf_map_extent_left.text()))
            ...
            self.config['debug'].debugLog("Info: process calculation for max extent")
        ...
        try:
            ...
            self.config['debug'].debugLog("impossible to remove ows_enable_request: it doesn't exist")
        try:
            self.map.removeMetaData('wms_enable_request')
        except:
            self.config['debug'].debugLog("impossible to remove wms_enable_request: it doesn't exist")
        try:
            self.map.removeMetaData('wfs_enable_request')
        except:
            self.config['debug'].debugLog( "impossible to remove wfs_enable_request: it doesn't exist")
    ...
    def setExtentFromLayers(self):
        ...
        for index in range(0,self.map.numlayers):
            self.config['debug'].debugLog("Extent: process for layer named" + layer.name)
    ...
    #TODO: create a projection class outside
    def getProjectionList(self):
        wktByCode = {}
        wktByName = {}
        nameByEpsg = {}
        epsgByWkt = {}
        epsgFile = open('src/epsg', 'r')
